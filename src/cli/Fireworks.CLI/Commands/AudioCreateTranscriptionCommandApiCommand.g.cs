...
");
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            await CliRuntime.RunAsync(async () =>
            {
                        ...
                        var diarize = CliRuntime.WasSpecified(parseResult, Diarize) ? parseResult.GetValue(Diarize) : (__requestBase is { } __DiarizeBaseValue ? __DiarizeBaseValue.Diarize : default);
                        var minSpeakers = CliRuntime.WasSpecified(parseResult, MinSpeakers) ? parseResult.GetValue(MinSpeakers) : (__requestBase is { } __MinSpeakersBaseValue ? __MinSpeakersBaseValue.MinSpeakers : default);
                        var maxSpeakers = CliRuntime.WasSpecified(parseResult, MaxSpeakers) ? parseResult.GetValue(MaxSpeakers) : (__requestBase is { } __MaxSpeakersBaseValue ? __MaxSpeakersBaseValue.MaxSpeakers : default);
